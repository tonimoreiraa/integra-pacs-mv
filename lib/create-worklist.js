...
export async function createWorklist(row, connection)
{
    ...
    const prestador = parseOracleRows(await connection.execute(`SELECT NM_PRESTADOR FROM PRESTADOR WHERE CD_PRESTADOR = ${pedido.CD_PRESTADOR}`))[0]
    ...
    const payload = {
        ...
        prestador_id: pedido.CD_PRESTADOR
    }
