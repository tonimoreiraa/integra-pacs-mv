...
export async function createWorklist(row, connection)
{
    ...
    const pedido = parseOracleRows(await connection.execute(`SELECT CD_ATENDIMENTO, HR_COLETA FROM PED_RX WHERE CD_PED_RX = ${row.CD_PED_RX}`))[0]
    ...
    const payload = {
        ...
        exame_datahora: pedido.HR_COLETA.toLocaleString('pt-BR').replace(',', ''),
        ...
        exame_tipo,
        accession_number: row.CD_PED_RX
    }
